<x-app-layout title="News">
    <x-content_header>
        <div class="col-sm-6">
            <h4>{{ isset($data) ? 'Edit' : 'Add' }} News</h4>
        </div>
        ...
        <x-breadcrumb>
            ...
            <li class="breadcrumb-item item active">{{ __( (isset($data) ? 'Edit' : 'Add') . ' News') }}</li>
        </x-breadcrumb>
    </x-content_header>
    <!-- Main content -->
    <section class="content">
        <div class="container-fluid">
            <form method="post" action="{{ isset($data) ? route('news.update', $data->id) : route('news.store') }}" enctype="multipart/form-data">
                @csrf
                @if (isset($data))
                    @method('PUT')
                @endif
