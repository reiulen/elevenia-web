...
const table = $("#example1").DataTable({
    ...
    serverSide: true,
    ajax: {
        url: `${url}/elevenia-admin/product-service/dataTable`,
        method: "POST",
        data: function (d) {
...
table.on("click", "a.btn-edit", function (e) {
    ...
    $.getJSON(`${url}/elevenia-admin/product-service/${id}`, function (res) {
        domStrings.formInputan.find('input[name="name"]').val(res?.data?.name);
        domStrings.formInputan.find('input[name="link"]').val(res?.data?.link);
        summernoteEditor.summernote('code', res?.data?.description);
...
table.on("click", ".btn-hapus", function (e) {
    ...
    let urlTarget = `${url}/elevenia-admin/product-service/${id}`;
